...
public class SyncAsyncExamples {
    ...
    private static void asynchronousMethods() {
        ...
        CompletableFuture<List<StudentDTO>> studentFuture =
                CompletableFuture.supplyAsync(() -> readStudentInfo("csv/StudentInfo.csv"));
        CompletableFuture<List<EmployeeDTO>> empFuture =
                CompletableFuture.supplyAsync(() -> readEmployeeInfo("csv/employee.csv"));
        CompletableFuture<List<Countries>> countriesFuture =
                CompletableFuture.supplyAsync(() -> readCountriesRegions("csv/CountriesRegions.csv"));
        CompletableFuture<List<SalesOrderDTO>> salesFuture =
                CompletableFuture.supplyAsync(() -> readSalesOrderDetails("csv/100000_Sales_Order.csv"));
        ...
        try {
            ...
            List<Countries> countryList = countriesFuture.get();
            List<SalesOrderDTO> saleList = salesFuture.get();
            System.out.println("CropSize==" + cropList.size() + "==StudentSize==" + stdList.size() +
                    "==EmpSize==" + empList.size() + "==CountrySize==" + countryList.size() + "==SalesSize===" + saleList.size());
    ...
    private static void synchronousMethods() {
        ...
        List<Countries> countryList = readCountriesRegions("csv/CountriesRegions.csv");
        List<SalesOrderDTO> saleList = readSalesOrderDetails("csv/100000_Sales_Order.csv");
        System.out.println("CropSize==" + cropDetails.size() + "==StudentSize==" + studentInfo.size() +
                "==EmpSize==" + employeeInfo.size() + "==CountrySize==" + countryList.size() + "==SalesSize===" + saleList.size());
        long endTime = System.currentTimeMillis();
        System.out.println("Total Time Taken===" + (endTime - startTime));
    }
